$(window).on("load", function () {
  ...
  $(".zoombox img").mousedown(function (e) {
    ...
    zoom.zoomboxLeft =
      $(zoombox).offset().left +
      parseInt($(zoombox).css("border-left-width").replace(/\D+/, ""));
    zoom.zoomboxTop =
      $(zoombox).offset().top +
      parseInt($(zoombox).css("border-top-width").replace(/\D+/, ""));
    ...
    if ($(".zoombox img").hasClass("drag")) {
      ...
      $(selector).css({
        ...
        width: 0,
        height: 0,
        left: zoom.cursorStartX,
        top: zoom.cursorStartY,
      });
    }
  });
    ...
    if ($(".zoombox img").hasClass("moving")) {
      if ($(".zoombox img").hasClass("drag")) {
        ...
        $(img).css({
          left:
            zoom.imgStartLeft +
            (e.pageX - zoom.zoomboxLeft) -
            zoom.cursorStartX,
          top:
            zoom.imgStartTop + (e.pageY - zoom.zoomboxTop) - zoom.cursorStartY,
        });
        //prevent dragging in prohibited areas (relative to zoombox)
        if ($(img).position().left <= zoom.minDragLeft) {
          $(img).css("left", zoom.minDragLeft);
        } else if ($(img).position().left >= zoom.maxDragLeft) {
          $(img).css("left", zoom.maxDragLeft);
        }
        if ($(img).position().top <= zoom.minDragTop) {
          $(img).css("top", zoom.minDragTop);
        } else if ($(img).position().top >= zoom.maxDragTop) {
          $(img).css("top", zoom.maxDragTop);
        }
      } else {
        ...
        //prevent dragging in prohibited areas (relative to zoombox)
        if (e.pageX - zoom.zoomboxLeft <= zoom.minDragLeft) {
          width = zoom.minDragLeft - zoom.cursorStartX;
        } else if (e.pageX - zoom.zoomboxLeft >= zoom.maxDragLeft) {
          width = zoom.maxDragLeft - zoom.cursorStartX;
        }
        if (e.pageY - zoom.zoomboxTop <= zoom.minDragTop) {
          height = zoom.minDragTop - zoom.cursorStartY;
        } else if (e.pageY - zoom.zoomboxTop >= zoom.maxDragTop) {
          height = zoom.maxDragTop - zoom.cursorStartY;
        }
        ...
        if (width < 0) {
          $(selector).css("left", zoom.cursorStartX - Math.abs(width));
        }
        if (height < 0) {
          $(selector).css("top", zoom.cursorStartY - Math.abs(height));
        }
      }
    }
  });
        ...
        if ($(selector).width() > 0 && $(selector).height() > 0) {
          ...
          var magnification =
            $(selector).width() < $(selector).height()
              ? $(selector).parent().width() / $(selector).width()
              : $(selector).parent().height() / $(selector).height(); //go for the highest magnification
          var hFactor =
            $(img).width() /
            ($(selector).position().left - $(img).position().left);
          var vFactor =
            $(img).height() /
            ($(selector).position().top - $(img).position().top);
          $(selector).css({
            width: $(selector).width() * magnification,
            height: $(selector).height() * magnification,
          });
          $(img).css({
            width: $(img).width() * magnification,
            height: $(img).height() * magnification,
          });
          ...
          var selectorLeft =
            $(selector).parent().width() / 2 - $(selector).width() / 2;
          var selectorTop =
            $(selector).parent().height() / 2 - $(selector).height() / 2;
          ...
          $(img).css({
            left: "+=" + selectorDeltaLeft,
            top: "+=" + selectorDeltaTop,
          });
        }
...
let actionInfo = [
  ...
  },
  {
    id: "act3",
    content: [
      "Deciaml Degrees = deg+<span class='frac'><sup>min</sup><span>&frasl;</span><sub>60</sub></span>+<span class='frac'><sup>sec</sup><span>&frasl;</span><sub>3600</sub></span><br><h3>NOTE:</h3><p>The georeferencing activity has to be done either in clockwise direction or anti-clockwise direction while choosing the points.</p><p>Minimum points required for georeferencing is 4. However, choosing more points (8 or 16) will increase the accuracy</p>",
    ],
    visibility: false,
  },
];
...
let coData = [
  {
    coId: 0,
    initialCoordLeft: "43px",
    initialCoordTop: "75px",
    finalCoordLeft: "59px",
    finalCoordTop: "75px",
    initialWidth: "auto",
    finalWidth: "45px",
    id: "tl",
    spanId: "tls",
    class: "coordTooltiptext",
    index: 3,
  },
  {
    coId: 1,
    initialCoordLeft: "457px",
    initialCoordTop: "75px",
    finalCoordLeft: "476px",
    finalCoordTop: "75px",
    initialWidth: "auto",
    finalWidth: "45px",
    id: "tr",
  ...
  {
    coId: 2,
    initialCoordLeft: "42px",
    initialCoordTop: "505px",
    finalCoordLeft: "56px",
    finalCoordTop: "502px",
    initialWidth: "auto",
    finalWidth: "45px",
  ...
  {
    coId: 4,
    initialCoordLeft: "503px",
    initialCoordTop: "265px",
    finalCoordLeft: "503px",
    finalCoordTop: "265px",
    initialWidth: "80px",
    finalWidth: "45px",
    id: "mr",
    spanId: "mrs",
    class: "rightTooltiptext",
    index: 4,
  },
  {
    coId: 5,
    initialCoordLeft: "253px",
    initialCoordTop: "505px",
    finalCoordLeft: "265px",
    finalCoordTop: "505px",
    initialWidth: "80px",
    finalWidth: "45px",
    id: "bm",
...
function lockAction1() {
  ...
  // document.getElementById("notify").style.animation= '';
  messagePopup(
    "Select the Projected Coordinate System and then the Northern Hemisphere is selected for Mangalore - WGS1984 UTM Zone43N"
  );
}
...
function showCoord(ele) {
  ...
  if (ele.value >= 0) {
    ...
    document.getElementById("zoomIn").style = `position: absolute; left: ${
      data[ele.value].zx
    }; top: ${data[ele.value].zy};visibility: visible`;
    document.getElementById("zSpan").style.visibility = "visible";
    document.getElementById("zSpan").innerHTML =
      "Coordinates: Zoom In Map here <img src='./images/zoomIn.png' width='23px'/>";
...
function addPoints() {
  ...
  if (validX == true && validY == true) {
    ...
    if (point <= 2) {
      ...
      document
        .getElementById("act3")
        .setAttribute("onclick", "showPopup(this)");
      document.getElementById("zSpan").innerHTML =
      ...
      setTimeout(function () {
        ...
        messagePopup(
          "The digital image is saved in.TIFF format and the saved “.TIFF” file is added to our workspace for further processing of the data. Go to Define Projections"
        );
        setTimeout(function () {
          document.getElementById("coordinatePopup").style.zIndex = "100";
          document.getElementById("zSpan").innerHTML = "";
    ...
    if (d.pt === 1 || d.pt === 2 || d.pt === 3 || d.pt === 4) {
      if (d.set == true) {
        document.getElementById(
          "mp" + d.pt
        ).style = `position: absolute; left: ${d.mx}; top: ${d.my};visibility: visible`;
      }
    }
  });
}
...
function setCoordinates(coId) {
  if (allSet === false) {
    document.getElementById(
      coData[coId].id
    ).style = `position: absolute; left: ${coData[coId].initialCoordLeft}; top: ${coData[coId].initialCoordTop}};visibility: visible`;
    document.getElementById(coData[coId].spanId).innerHTML = `<p>x: ${data[
      coData[coId].index
    ].xi.toFixed(2)}</p> <p>y: ${data[coData[coId].index].yi.toFixed(2)}</p>`;
    ...
    document.getElementById(
      coData[coId].id
    ).style = `position: absolute; left: ${coData[coId].finalCoordLeft}; top: ${coData[coId].finalCoordTop}};visibility: visible`;
    document.getElementById(coData[coId].spanId).innerHTML = `<p>x: ${data[
      coData[coId].index
    ].x.toFixed(2)}</p> <p>y: ${data[coData[coId].index].y.toFixed(2)}</p>`;
...
function generalInstructions() {
  document.getElementById("tooltiptext").style.visibility = "visible";
  document.getElementById("tooltiptext").innerHTML = `
    <p>Click on <button class="buttonStyle1"></button> to enable zoom in by drag and selecting area in the map.</p>
    <p>Click on <button class="buttonStyle2"></button> to zoom out (reset) map and enable coordinates view.</p>
  ...
  if (inference == 1) {
    setTimeout(function () {
      document.getElementById("notify").innerHTML =
        "INFERENCE: Georeferencing is a method by which mere images like maps or aerial photos can be easily related to the actual ground system of geographic coordinates for use in a variety of circumstances.";
      setTimeout(function () {
